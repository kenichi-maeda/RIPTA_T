...
final class GTFSStaticDataLoader {
    ...
    private init() {
        agencies        = Self.loadCSV("agency",          ext: "txt", subdirectory: nil, parser: GTFSStaticDataLoader.parseAgency)
        routes          = Self.loadCSV("routes",          ext: "txt", subdirectory: nil, parser: GTFSStaticDataLoader.parseRoute)
        trips           = Self.loadCSV("trips",           ext: "txt", subdirectory: nil, parser: GTFSStaticDataLoader.parseTrip)
        stops           = Self.loadCSV("stops",           ext: "txt", subdirectory: nil, parser: GTFSStaticDataLoader.parseStop)
        stopTimes       = Self.loadCSV("stop_times",      ext: "txt", subdirectory: nil, parser: GTFSStaticDataLoader.parseStopTime)
        calendarDates   = Self.loadCSV("calendar_dates",  ext: "txt", subdirectory: nil, parser: GTFSStaticDataLoader.parseCalendarDate)
        feedInfo        = Self.loadCSV("feed_info",       ext: "txt", subdirectory: nil, parser: GTFSStaticDataLoader.parseFeedInfo)
        routeTimepoints = Self.loadCSV("route_timepoints",ext: "txt", subdirectory: nil, parser: GTFSStaticDataLoader.parseRouteTimepoint)
        shapePoints     = Self.loadCSV("shapes",          ext: "txt", subdirectory: nil, parser: GTFSStaticDataLoader.parseShapePoint)
    }
    ...
    private static func loadCSV<T>(
        ...
        // ① Print where Bundle thinks its resources live
        if let resURL = Bundle.main.resourceURL {
            print("[GTFS] resourceURL:", resURL.path)
        }
        // ② Attempt to locate the file
        let foundURL = Bundle.main.url(
            forResource: resource,
            withExtension: ext,
            subdirectory: subdirectory
        )
        print("[GTFS] looking for \(resource).\(ext) in \(subdirectory ?? "root") →",
              foundURL?.path ?? "NOT FOUND")

        // ③ If missing or unreadable, warn and return empty array
        guard
          let url     = foundURL,
          let content = try? String(contentsOf: url, encoding: .utf8)
        else {
          print("⚠️ GTFSStaticDataLoader: couldn’t load \(resource).\(ext)")
          return []
        }
